...
def dynamic_topK(A, k=10):
    """
    A [batch, node, node]
    """
    mask = torch.zeros_like(A).to(A.device)
    mask.fill_(float('0'))
    s1, t1 = (A + torch.rand_like(A)*0.01).topk(k, 2)
...
def topK(A, k=10):
    ...
    mask = torch.zeros_like(A).to(A.device)
    mask.fill_(float('0'))
    s1, t1 = (A + torch.rand_like(A)*0.01).topk(k, 1)
...
if __name__ == '__main__':
    ...
    t = torch.FloatTensor(t).to("cpu")
    print(t)
    print(dynamic_topK(t, 1))